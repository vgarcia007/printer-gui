...
Before physical acceptance, run make validate and confirm that all four containers are healthy. Then use the browser at port 8081 to perform one controlled test each for document printing, a saved DYMO label, a front-only scan, a two-sided scan, and a color scan.
